...
is_mc_server_already_running(){
	...
	if screen -ls minecraft; then
		server_running_state="true"
	elif screen -ls spigot; then
		server_running_state="true"
	fi
...
start_server() {
	...
	if is_mc_server_already_running == "false"; then
		...
		if ! screen -A -m -d -S minecraft java -Xms1024M -Xmx12G -XX:ParallelGCThreads=4 -XX:MinHeapFreeRatio=5 -XX:MaxHeapFreeRatio=10 -jar $SERVER_HOME_FOLDER/$SERVER_JARFILE_NAME nogui; then
			return 1
		fi
			return 0
